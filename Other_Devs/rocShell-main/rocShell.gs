...
getCloudExploitAPI = function(metaxploit) //cloud exploit database api.
    ...
    netSession = metaxploit.net_use(nslookup("www.ExploitDatabase.org"), 80) //connect to server with metaxploit on ssh service
    if netSession then metaLib = netSession.dump_lib else metaLib = null
    if metaLib then remoteShell = metaLib.overflow("0x292F648B", "yn_dtreedyn_ltree") else remoteShell = null //exploit needed to grab a guest shell to the server
...
getRshellAPI = function(metaxploit)
    ...
    if typeof(metaxploit) != "MetaxploitLib" then return print("metaxploit required for api to work.")
    netSession = metaxploit.net_use(nslookup("www.OpenRshell.org"), 80) //connect to server with metaxploit on ssh service
    if netSession then metaLib = netSession.dump_lib else metaLib = null
    if metaLib then remoteShell = metaLib.overflow("0x292F648B", "yn_dtreedyn_ltree") else remoteShell = null //exploit needed to grab a guest shell to the server
    if typeof(remoteShell) != "shell" then print("Server failed. API running in local mode.")
...
current.compName = function
    if typeof(current.obj) == "shell" then return current.obj.host_computer.get_name
    if typeof(current.obj) == "computer" then return current.obj.get_name
    return "<i>FILE</i>"
end function
...
computerCommands["wifi"]["run"] = function(args)
    if args.len < 4 then return print("Usage: wifi [device] [bssid] [essid] [password]")
    computer = current.computer
...
shellCommands["ssh"]["run"] = function(args)
    ...
    globals.objects.push({"object":remoteShell, "user":libs.checkAccess(libs.toFile(remoteShell)), "localIp":remoteShell.host_computer.local_ip, "publicIp":remoteShell.host_computer.public_ip, "router":get_router(sshIp)})
...
shellCommands["brutessh"]["run"] = function(args)
    ...
    for pas in hashMap.values
        ...
        globals.objects.push({"object":result, "user":libs.checkAccess(libs.toFile(result)), "localIp":result.host_computer.local_ip, "publicIp":result.host_computer.public_ip, "router":get_router(ip)})
...
commands["nmap"]["run"] = function(args) //thanks to Nameless for this awesome nmap. I am too lazy to write a new one. It is MIT licensed anyway. //update 20240302: i did write a new one.
    ...
    if is_lan_ip(targetIp) then
        if get_router(targetIp) then
            router = get_router(targetIp)
        else if get_switch(targetIp) then
            router = get_switch(targetIp)
        else
            router = current.router
        end if
    else
        if not get_router(targetIp) then return print("Invalid ip.")
        router = get_router(targetIp)
    end if
    ...
    nmapInfo = "Exposed ports on router " + router.public_ip + " " + router.local_ip
